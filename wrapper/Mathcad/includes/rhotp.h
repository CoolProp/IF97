// if97_rhotp stub - Interfaces CoolProp IF97 function to Mathcad
//
...
{  
    ...
    try {
        c->real = IF97::rhomass_Tp(a->real,b->real);
    }
...
FUNCTIONINFO    if97_rhotp = 
{
    "if97_rhotp",                    // name by which Mathcad will recognize the function
    "T,p",                             // if97_rhotp will be called as if97_rhotp(T,p)
    // description of if97_rhotp(p)
    "Obtains the mass density [kg/m^3] as a function of T [K] and p [Pa].",
    (LPCFUNCTION)if97_RhoTP,         // pointer to executable code
